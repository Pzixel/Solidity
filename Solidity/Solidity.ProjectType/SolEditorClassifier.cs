    ...
    internal class SolKeyword : IClassifier
    {
        ...
        internal SolKeyword(IClassificationTypeRegistryService registry)
        {
            _map = new List<(Regex, IClassificationType)>
            {
                ...
                (new Regex($@"\b({Keywords}|{BuiltinTypes})\b", RegexOptions.Compiled), registry.GetClassificationType(Classification.SolKeyword)),
                (new Regex($@"\b({Operators})\b", RegexOptions.Compiled), registry.GetClassificationType(Classification.SolKeyword))
            };
        }
